...
{
    float noise = Turbulence(position * NoiseFrequency + Animation*u_time, 0.1, 1.5, 0.03) * NoiseAmplitude;
...
void main() {
    ...
    // vec3 _brigtness = vec3(r, g, b) / 3.0;

    // outputColor = vec4(r, g, b, length(_brigtness));
//
//    vec2 p = texCoordVaring;
//    p.x *= 1.3333;
//    float rotx = 0.0;
//    float roty = 0.0;
//    float zoom = 5.0;
